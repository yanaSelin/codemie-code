...
export async function registerGeminiSkill(
  skill: SkillDetail,
  ...
  logger.debug('Registered Gemini skill', { slug, skillFile });
  return slug;
}

export async function unregisterGeminiSkill(
  slug: string,
  scope: 'global' | 'local' = 'global',
  workingDir?: string
): Promise<void> {
  ...
  logger.debug('Unregistered Gemini skill', { slug, skillDir });
}
